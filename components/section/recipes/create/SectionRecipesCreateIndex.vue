...
import type { StepField, IngredientField } from "~/interfaces/recipes";
...
const ingredientOptions = ref<SelectOption[]>([])
...
async function fetchIngredients(query: string) {
  ...
  try {
    ...
    ingredientOptions.value = res.data.value.data.map((item: any) => ({
      label: item.name,
      value: item.id
    }))
  } catch (e) {
...
<template>
  <div class="page-container create-recipe-page">
    <n-form @submit.prevent="submitForm" label-placement="top">
      ...
      <n-card title="Ингредиенты" class="card">
        ...
        <div class="actions">
          <n-button type="primary" ghost @click="addIngredient({ id: null, weight_grams: null })">
            Добавить ингредиент
          </n-button>
        </div>
      </n-card>
          ...
          >
            ...
            >
              <n-upload
                  ...
                  :max="1"
                  accept="image/png, image/jpeg, image/webp"
                  :on-change="(options) => handleStepUpload(step, options)"
                  :on-remove="() => handleStepRemove(step)"
              >
                Добавить фото
              </n-upload>
            </n-form-item>
